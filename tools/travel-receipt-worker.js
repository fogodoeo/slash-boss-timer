...
const RECEIPT_OUTPUT_CONTRACT = [
    ...
    'lineItems is for visible menu/product/fare detail. Extract up to 8 readable line items with their individual amounts when the receipt shows them. Look for itemized rows above the total, even if they are small or partly Japanese/Korean.',
    ...
    'If a row has a readable product/menu name but no individual amount, include the name with amount 0.',
    'If a row has a readable amount but the name is partially unreadable, include the best short label and the amount.',
    'If the receipt is only a card sales slip or the uploaded image truly does not show itemized rows, return an empty lineItems array and explicitly say so in aiNote.',
